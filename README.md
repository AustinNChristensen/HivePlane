...
Still on the Hive machine (or anywhere with the admin token):
...
From any machine that can reach the Hive:

```bash
curl http://mac-mini.tailnet-name.ts.net:8787/api/bees | jq
```

You should see one entry with your `beeId`, status `online`, and `heartbeatCount` increasing every 10 seconds. On the Bee, `hive status` shows the same info plus session details and service state.
...
The Bee picks it up on the next heartbeat (≤10s), runs it, and posts a completion. Check the result:
